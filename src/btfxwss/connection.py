...
class WebSocketConnection(Thread):
    ...
    def disconnect(self):
        ...
        :return:
        """
        log.debug("disconnect(): Disconnecting from API..")
    ...
    def reconnect(self):
        ...
        log.debug("reconnect(): Initialzion reconnect sequence..")
    ...
    def _connect(self):
        ...
        :return:
        """
        log.debug("_connect(): Initializing Connection..")
        ...
        log.debug("_connect(): Starting Connection..")
        ...
        while self.reconnect_required.is_set():
            if not self.disconnect_called.is_set():
                log.info("Attempting to connect again in %s seconds."
    ...
    def _on_open(self, ws):
        ...
        if self.reconnect_required.is_set():
            log.info("_on_open(): Connection reconnected, re-subscribing..")
    ...
    def _stop_timers(self):
        ...
        log.debug("_stop_timers(): Timers stopped.")
    ...
    def _start_timers(self):
        ...
        :return:
        """
        log.debug("_start_timers(): Resetting timers..")
        self._stop_timers()
    ...
    def send_ping(self):
        ...
        :return:
        """
        log.debug("send_ping(): Sending ping to API..")
    ...
    def _check_pong(self):
        ...
        if self.pong_received:
            log.debug("_check_pong(): Pong received in time.")
            self.pong_received = False
        else:
            # reconnect
            log.debug("_check_pong(): Pong not received in time."
                           "Issuing reconnect..")
            self.reconnect()
    ...
    def send(self, api_key=None, secret=None, list_data=None, auth=False, **kwargs):
        ...
        log.debug("send(): Sending payload to API: %s", payload)
        try:
            self.socket.send(payload)
        except websocket.WebSocketConnectionClosedException:
            log.error("send(): Did not send out payload %s - client not connected. ", kwargs)
    ...
    def _connection_timed_out(self):
        ...
        :return:
        """
        log.debug("_connection_timed_out(): Failed! Issuing reconnect..")
        self.reconnect()
    ...
    def _pause(self):
        ...
        :return:
        """
        log.debug("_pause(): Setting paused() Flag!")
        self.paused.set()
    ...
    def _unpause(self):
        ...
        :return:
        """
        log.debug("_unpause(): Clearing paused() Flag!")
        self.paused.clear()
        log.debug("_unpause(): Re-subscribing softly..")
    ...
    def _heartbeat_handler(self):
        ...
        log.debug("_heartbeat_handler(): Received a heart beat "
                       "from connection!")
        self._start_timers()
    ...
    def _pong_handler(self):
        ...
        log.debug("_pong_handler(): Received a Pong message!")
    ...
    def _system_handler(self, data, ts):
        ...
        if event == 'pong':
            ...
            log.error("Unhandled event: %s, data: %s", event, data)
    ...
    def _response_handler(self, event, data, ts):
        ...
        :return:
        """
        log.debug("_response_handler(): Passing %s to client..", data)
    ...
    def _info_handler(self, data):
        ...
        if 'version' in data:
            log.info("API version: %i", data['version'])
            return

        try:
            log.info(info_message[data['code']])
            codes[data['code']]()
        except KeyError as e:
            log.exception(e)
            log.error("Unknown Info code %s!", data['code'])
            raise
    ...
    def _error_handler(self, data):
        ...
        try:
            log.error(errors[data['code']])
        except KeyError:
            log.error("Received unknown error Code in message %s! "
                           "Reconnecting..", data)

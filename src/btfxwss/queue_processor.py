...
class QueueProcessor(Thread):
        ...
        while not self._stopped.is_set():
            ...
            if dtype in ('subscribed', 'unsubscribed', 'conf', 'auth', 'unauth'):
                try:
                    ...
                    log.error("Dtype '%s' does not have a response "
                ...
                try:
                    ...
                    log.error("Channel ID does not have a data handler! %s",
                                   message)
            else:
    ...
    def _handle_subscribed(self, dtype, data, ts,):
        ...
        :return:
        """
        log.debug("_handle_subscribed: %s - %s - %s", dtype, data, ts)
    ...
    def _handle_unsubscribed(self, dtype, data, ts):
        ...
        :return:
        """
        log.debug("_handle_unsubscribed: %s - %s - %s", dtype, data, ts)
        channel_id = data.pop('chanId')
        ...
        log.info("Successfully unsubscribed from %s", chan_identifier)
    ...
    def _handle_conf(self, dtype, data, ts):
        ...
        :return:
        """
        log.debug("_handle_conf: %s - %s - %s", dtype, data, ts)
        log.info("Configuration accepted: %s", dtype)
        return
    ...
    def update_timestamps(self, chan_id, ts):
        ...
        try:
            ...
            log.warning("Attempted ts update of channel %s, but channel "
                             "not present anymore.",
    ...
    def _handle_account(self, data, ts):
        ...
        chan_id, event, payload = data
        entry = (event, payload, ts)
    ...
    def _handle_ticker(self, dtype, data, ts):
        ...
        :return:
        """
        log.debug("_handle_ticker: %s - %s - %s", dtype, data, ts)
        channel_id, data = data
        channel_identifier = self.channel_directory[channel_id]
        self._put_data_to_queue(data, ts, self.tickers[channel_identifier])
    ...
    def _handle_raw_book(self, dtype, data, ts):
        ...
        :return:
        """
        log.debug("_handle_raw_book: %s - %s - %s", dtype, data, ts)
        channel_id, data = data
        channel_identifier = self.channel_directory[channel_id]
        self._put_data_to_queue(data, ts, self.raw_books[channel_identifier])
    ...
    def _handle_trades(self, dtype, data, ts):
        ...
        :return:
        """
        log.debug("_handle_trades: %s - %s - %s", dtype, data, ts)
        channel_id, data = data
        channel_identifier = self.channel_directory[channel_id]
        self._put_data_to_queue(data, ts, self.trades[channel_identifier])
    ...
    def _handle_candles(self, dtype, data, ts):
        ...
        :return:
        """
        log.debug("_handle_candles: %s - %s - %s", dtype, data, ts)
        channel_id, data = data
        channel_identifier = self.channel_directory[channel_id]
        self._put_data_to_queue(data, ts, self.candles[channel_identifier])

    def _put_data_to_queue(self, data, ts, queue):
